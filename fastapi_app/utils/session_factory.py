...
def create_agent_session(conversation_id: str, ttl: Optional[int] = None):
    """
    ...
    if SESSION_BACKEND == "redis":
        try:
            ...
            logger.info(f"Creating Redis session for conversation {conversation_id}")
    ...
    try:
        ...
        logger.info(f"Creating PostgreSQL session for conversation {conversation_id}")
